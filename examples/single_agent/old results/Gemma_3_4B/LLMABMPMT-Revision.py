...
# --- MODIFIED FOR PROMPT REVISION (2025/12/02) By Wenyu Chiou ---
# - Revise persona to prioritize safety and financial stability over community attachment
# - Update elevation status text to explicitly state 80% risk reduction and residual risk
...
MAX_RETRIES = 5                     # Number of times to retry a failed LLM batch
RETRY_DELAY_SECONDS = 20             # Seconds to wait between retries
...
#2a. Randomly generate agents' attribute 
...
#2b. Load agents' attribute from csv file
def load_agents_from_file(filepath, past_events):
    """Loads agent profiles from a CSV file."""
    df = pd.read_csv(filepath)
    agents = []
    for _, row in df.iterrows():
        # Handle case where memory might be missing or empty in the file
...
def determine_flood_exposure(agent, year, flood_event):
    ...
    flooded = False # either flood does not occur or agent is not impacted
    if flood_event and not agent["elevated"]:
        # Agent is not elevated, so they are vulnerable
        if random.random() < agent["flood_threshold"]:
            flooded = True # this means the specific agent has been impacted
            agent["memory"].append(f"Year {year}: Got flooded with $10,000 damage on my house.")
            # Being flooded and having insurance slightly erodes trust due to the hassle
        ...
        # Check against reduced threshold
        if random.random() < agent["flood_threshold"]:
            flooded = True
            ...
            # Agent is protected by elevation
...
You are a homeowner in a city. You care about your community, but your primary priority is the safety and financial stability of your household. {elevation_status_text}
...
def parse_response(response):
    ...
    for line in lines:
        if "Threat Appraisal:" in line: threat = line.split("Threat Appraisal:")[-1].strip()
            ...
            # Find the first valid decision code in the line
            for code in ["1", "2", "3", "4"]:
                if code in line:
                    decision = code
                    break
...
# --- Conditional setup: Load from files or generate randomly ---
    ...
    # Choose five agents and three years to record sample prompts
    ...
    log_file_path = "flood_adaptation_simulation_log.csv"
    pd.DataFrame(columns=[
        "agent_id", "year", "decision", "raw_llm_code", "raw_llm_decision", 
        "threat_appraisal", "coping_appraisal", "memory", "trust_in_insurance", 
        "trust_in_neighbors", "elevated", "has_insurance", "relocated", "flooded_this_year"
    ]).to_csv(log_file_path, index=False, encoding="utf-8-sig")
    ...
    with open("example_llm_prompts.txt", "w", encoding="utf-8") as prompt_file:
        ...
        for year in tqdm(range(1, NUM_YEARS + 1), desc="Simulating Years"):
            ...
            # Pre-calculate neighborhood stats (O(N) operation)
            ...
            # Prepare prompts and corresponding agents for processing
            tasks_to_process = []
            current_year_logs = [] # Temp list for this year's logs
            ...
            for agent in active_agents:
                ...
                # Add neighborhood stats to memory
                ...
                if agent["elevated"]:
                    prompt_vars.update({
                        "elevation_status_text": "Your house is elevated, which has reduced your flood risk by 80%. However, extreme floods can still cause damage (residual risk).",
            ...
            for i in tqdm(range(0, len(tasks_to_process), BATCH_SIZE), desc=f"Year {year} Batches", leave=False):
                ...
                for task, response in zip(task_chunk, responses_chunk):
                    ...
                    # Map raw_llm_decision based on context at decision time
                    if was_elevated:
                    ...
                    # Update states (elevation irreversible, insurance annual)
                    if was_elevated:
                        if decision_code == "1":
                        ...
                        if decision_code == "1":
                            ...
                            agent["elevated"] = True
                            # --- 80% RISK REDUCTION due to house evelation---
                    ...
                    agent["trust_in_insurance"] = max(0.0, min(1.0, agent["trust_in_insurance"] + random.uniform(-0.03, 0.03)))
                    agent["trust_in_neighbors"] = max(0.0, min(1.0, agent["trust_in_neighbors"] + random.uniform(-0.02, 0.02)))
                    
                    # Add to current year logs
                    current_year_logs.append({
                        "agent_id": agent["id"], "year": year,
                        "decision": adaptation_state,                 # combined category
                        "raw_llm_code": decision_code,                # numeric code from LLM
                        "raw_llm_decision": raw_llm_decision,         # mapped textual action
                    ...
                    # Add to main logs list (kept for plotting at the end)
                    logs.append(current_year_logs[-1])
            
            # Log status for previously relocated agents
            for agent in agents:
                if agent["relocated"] and len(agent["yearly_decisions"]) < year:
                    agent["yearly_decisions"].append("Already relocated")
                    log_entry = {"agent_id": agent["id"], "year": year, "decision": "Already relocated"}
                    current_year_logs.append(log_entry)
                    logs.append(log_entry)

            # --- INCREMENTAL SAVE: Append this year's logs to CSV ---
            df_year = pd.DataFrame(current_year_logs)
            df_year.fillna({"threat_appraisal": "N/A", "coping_appraisal": "N/A"}, inplace=True)
            # Append to file without header
            df_year.to_csv(log_file_path, mode='a', header=False, index=False, encoding="utf-8-sig")
            print(f"  Saved logs for Year {year}.")
...
if __name__ == "__main__":
    ...
    # df_log.to_csv("flood_adaptation_simulation_log.csv", index=False, encoding="utf-8-sig") # Already saved incrementally
    ...
    for agent_id in agent_ids_to_plot:
        ...
        # Line of combined adaptation state
        ...
        # Shade community-wide flood years
