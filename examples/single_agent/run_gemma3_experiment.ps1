...
$Models = @(
    @{ Tag = "gemma3:1b";  Name = "gemma3_1b" },
    @{ Tag = "gemma3:4b";  Name = "gemma3_4b" },
...
Write-Host "Groups: A (Original), B (Strict), C (Full Cognitive)" -ForegroundColor Yellow
Write-Host "Params: years=$Years, agents=$Agents, seed=$Seed, num-ctx=$NumCtx, num-predict=$NumPredict" -ForegroundColor Yellow
Write-Host "==========================================" -ForegroundColor Cyan

$TotalRuns = $Models.Count * 3  # 3 groups per model
$CurrentRun = 0

foreach ($Model in $Models) {

    # ===== GROUP A: Original LLMABMPMT-Final.py (No Broker Framework) =====
    $CurrentRun++
    $OutputDir = "$SAPath/results/JOH_FINAL/$($Model.Name)/Group_A/Run_1"
    $Timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'

    Write-Host "`n[$Timestamp] --------------------------------------------------" -ForegroundColor Green
    Write-Host "[$CurrentRun/$TotalRuns] $($Model.Name) | Group_A (Original Baseline)" -ForegroundColor Green
    Write-Host "  Script: LLMABMPMT-Final.py (no governance, no broker)" -ForegroundColor DarkGray
    ...
    $BaseCmd = "python $RefPath/LLMABMPMT-Final.py --model $($Model.Tag) --years $Years --agents $Agents --seed $Seed --agents-path `"$SAPath/agent_initial_profiles.csv`" --flood-years-path `"$SAPath/flood_years.csv`" --output $OutputDir"
    $cmd = "cmd /c $BaseCmd 2>&1"
    Invoke-Expression "$cmd | Tee-Object -FilePath `"$LogFile`""

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [SUCCESS] $($Model.Name) Group_A Finished." -ForegroundColor Green
    } else {
        Write-Host "  [FAILURE] $($Model.Name) Group_A Failed. Check: $LogFile" -ForegroundColor Red
    }
