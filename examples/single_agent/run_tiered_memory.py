# -*- coding: utf-8 -*-
# =============================================================================
# run_tiered_memory.py - Tiered Memory Flood Adaptation with LLM via Ollama
# (Group C Implementation - Cumulative Adaptation Support)
# =============================================================================
# This script implements a "Tiered Memory" system where:
# 1. Short-Term Memory: A sliding window of the last 5 relevant observations.
# 2. Long-Term Memory: A permanent record of significant events (Floods, Backstory).
# =============================================================================
...
MEMORY_WINDOW = 5                   # Number of recent memories an agent retains in Short-Term Memory
...
# --- 2. Initialize the agents with random attributes and tiered memory ---
def initialize_agents_randomly(past_events):
    """Creates the initial population of agents with Tiered Memory."""
    agents = [] # List to hold all initialized agents
    for i in range(NUM_AGENTS):
        # Backstory is treated as Long-Term Memory
        backstory = random.sample(past_events, k=random.randint(2, 3))
        agent = {
            "id": f"Agent_{i+1}",
            "elevated": False,
            "has_insurance": random.choice([True, False]),
            "relocated": False,
            "memory": backstory[:], # Short-term starting with backstory
            "long_term_memory": backstory[:], # Backstory is permanently stored
...
def load_agents_from_file(filepath, past_events):
    """Loads agent profiles and initializes Tiered Memory."""
...
# --- 3. Agent-specific flood exposure calculation with Tiered Memory ---
def determine_flood_exposure(agent, year, flood_event):
    """Determines if an agent is flooded and updates both memory tiers."""
    flooded = False
    memory_entry = ""
    if flood_event and not agent["elevated"]:
        if random.random() < agent["flood_threshold"]:
            flooded = True
            memory_entry = f"Year {year}: Got flooded with $10,000 damage on my house."
        else:
            memory_entry = f"Year {year}: A flood occurred, but my house was spared damage."
    elif flood_event and agent["elevated"]:
        if random.random() < agent["flood_threshold"]:
            flooded = True
            memory_entry = f"Year {year}: Despite elevation, the flood was severe enough to cause damage."
        else:
            memory_entry = f"Year {year}: A flood occurred, but my house was protected by its elevation."
    elif not flood_event:
        memory_entry = f"Year {year}: No flood occurred this year."
    ...
    # Update Long-Term Memory (Only Significant Events: Actual Floods or Protective Successes)
    if flood_event:
        agent["long_term_memory"].append(memory_entry)
        
    return flooded

# --- 4. Define the LLM prompt using Tiered Memory Structure ---
...
def run_simulation(model_name, output_dir):
    ...
    if os.path.exists(AGENT_PROFILE_FILE) and os.path.exists(FLOOD_YEARS_FILE):
        ...
        # Save initial profile for logging/audit
    ...
    selected_agent_ids = [f"Agent_{i+1}" for i in random.sample(range(NUM_AGENTS), min(NUM_AGENTS, NUM_SAMPLED_AGENTS))]
    sampled_years = random.sample(range(1, NUM_YEARS + 1), min(NUM_YEARS, NUM_SAMPLED_YEARS))
    ...
    with open(output_path / "example_llm_prompts.txt", "w", encoding="utf-8") as prompt_file:
        for year in tqdm(range(1, NUM_YEARS + 1), desc="Simulating Years"):
            ...
            for agent in active_agents:
                ...
                # Contextual memory updates (Short-Term only)
                ...
                prompt_vars = {
                    "long_term_memory": "\n".join(f"- {item}" for item in agent["long_term_memory"]),
            ...
            for i in tqdm(range(0, len(tasks_to_process), BATCH_SIZE), desc=f"Year {year} Batches", leave=False):
                ...
                for task, response in zip(task_chunk, responses_chunk):
                    ...
                    logs.append({
                        ...
                        "long_term_memory": " | ".join(agent["long_term_memory"]),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="results/tiered_memory")
    parser.add_argument("--agents", type=int, default=100)
    parser.add_argument("--years", type=int, default=10)
    args = parser.parse_args()

    # Apply arguments to global parameters
    ...
    plt.title("Adaptation States by Year (Tiered Memory)")
    ...
    for agent_id in agent_ids_to_plot:
        ...
        ax.set_title(f"Yearly Adaptation Decisions for {agent_id} (Tiered Memory)")
        ax.legend()
        plt.tight_layout()
