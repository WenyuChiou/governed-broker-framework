...
from agents.base_agent import BaseAgent, AgentConfig, PerceptionSource, Skill
...
class FloodSimulation(BaseSimulationEngine):
    ...
    def __init__(self, num_agents: int = 100, seed: int = 42, flood_years: List[int] = None):
        super().__init__() # Initialize Base (TieredEnvironment)
        self.seed = seed
        random.seed(seed)
        
        self.agents: Dict[str, BaseAgent] = {}
        # Legacy environment dict (keep for compat with Flood Logic)
        self.environment = {
            'year': 0, 
            'flood_event': False, 
            'flood_severity': 0.0
...
def run_experiment(args):
    ...
    # Initialize Environment & World Model
    sim.env.set_global("market_price", 100.0)
    sim.env.set_global("market_trend", "stable")
    sim.env.set_global("supply", 1000)
    
    from examples.single_agent.world_models.pricing_model import PricingModel
    pricing_model = PricingModel(sim.env)
    
    # Main simulation loop
    for year in range(1, args.num_years + 1):
        # 1. Update World Model (Market Dynamics)
        # In a real loop, we would pass all agents. Here we pass the single agent.
        pricing_model.step(list(sim.agents.values()))
        
        # Get Observable State
        market_price = sim.env.get_observable("global.market_price")
        market_trend = sim.env.get_observable("global.market_trend")
        
        print(f"\n--- Year {year} ---")
        print(f" Market Price: ${market_price:.2f} ({market_trend})")
        
        # 2. Update Agent Memory & Perception
        for agent in sim.agents.values():
            # Observe Price
            memory_engine.add_memory(agent.id, f"Year {year}: Market price is ${market_price:.2f}. Trend is {market_trend}.")
            
            # Observe Budget (Personal State)
            budget = agent.dynamic_state.get("budget", 1000.0)
            
            # Context specific to Pricing
            # We bypass the specific 'FloodContextBuilder' for this demo, 
            # or we would need to adapt it. For now, we inject a simple prompt extension.
            # (In a real app, we'd enable a PricingContextBuilder)
            
        # 3. Process Decision
        for agent in sim.agents.values():
            step_counter += 1
            
            # For this demo, we are re-using the 'Instruction' prompt from household
            # But normally we'd swap the Agent Config to a 'Consumer' type.
            # We will rely on the LLM adapting to the 'memory' context of prices.
            
            # Creating a mock 'market' skill set for the agent just for this runtime
            # This is a hack for the demo script without changing agent_types.yaml yet
            agent.config.skills = [
                Skill(skill_id="buy", description="Buy unit", affected_param="budget", direction="decrease"),
                Skill(skill_id="wait", description="Wait for better price", affected_param="budget", direction="none")
            ]
            ...
            result = broker.process_step(
                ...
                agent_type="household" # We keep the persona but change the context
            )
            
            # Store decision for the World Model's next step
            # We simplistically parse the result string or assume 'wait' if unclear
            decision = "wait"
            
            # Extract decision string safely
            decision_str = ""
            if result.approved_skill:
                decision_str = result.approved_skill.skill_name
            elif result.skill_proposal:
                decision_str = result.skill_proposal.skill_name
            
            if "buy" in decision_str.lower():
                decision = "buy"
            ...
            log_entry = {
                ...
                "budget": agent.dynamic_state.get("budget"),
                "decision": decision_str,
                "reasoning": result.skill_proposal.reasoning if result.skill_proposal else ""
            }
            logs.append(log_entry)
            print(f" Agent {agent.id} decided: {decision_str}")

    # Save logs
    import json
    with open(output_dir / "pricing_audit.jsonl", "w") as f:
        for log in logs:
