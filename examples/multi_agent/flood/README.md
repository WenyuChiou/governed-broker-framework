...
│   │
│   ├── fig_agent_spatial_distribution.py # Agent map visualization
│   ├── compute_validation_metrics.py     # L1/L2 validation computation
│   ├── persona_sensitivity.py            # L3 persona sensitivity test
│   ├── prompt_sensitivity.py             # L3 prompt sensitivity test
│   ├── empirical_benchmarks.py           # Benchmark definitions
│   └── generate_wrr_tables.py            # WRR paper tables
│
└── results/
    ├── cv/                               # L3 validation (pre-experiment)
    │   ├── icc_report.json              # ICC(2,1), eta-squared
    │   ├── icc_responses.csv            # 2,700 LLM responses
    ...
    │   ├── l1_micro_metrics.json        # CACR, R_H, EBE
    │   ├── l2_macro_metrics.json        # EPI, benchmark results
...
|------|---------|----------------|
| `cv/icc_report.json` | ICC(2,1), eta², pass status | Pre-experiment |
| `cv/persona_sensitivity_report.json` | 4 swap test results | Pre-experiment |
| `validation/l1_micro_metrics.json` | CACR, R_H, EBE per agent type | Post-experiment |
| `validation/l2_macro_metrics.json` | EPI, 8 benchmark comparisons | Post-experiment |
| `validation/benchmark_comparison.csv` | Benchmarks vs empirical ranges | Post-experiment |
...
> **Note**: For complete output path specifications, see [paper3/OUTPUT_PATHS.md](paper3/OUTPUT_PATHS.md).
...
Agent spatial distribution and flood overlay figures are generated by the Paper 3 analysis scripts:

```text
paper3/analysis/figures/
├── agent_spatial_distribution.png       # Main map: 400 agents + flood depth
├── agent_spatial_distribution.pdf       # Vector version for publication
├── agent_spatial_distribution_panels.png # 4-panel by agent type
├── agent_spatial_distribution_panels.pdf
└── agent_distribution_stats.csv         # Statistics table
```
...
```bash
python paper3/analysis/fig_agent_spatial_distribution.py --year 2011  # Specific flood year
python paper3/analysis/fig_agent_spatial_distribution.py --no-flood   # Without flood overlay
```

**Output specifications**: 300 DPI, 10×8 inches, serif fonts (WRR publication standard).
