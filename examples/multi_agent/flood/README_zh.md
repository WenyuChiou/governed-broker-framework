...
| 來源 | 用途 | 數量 | 類型 |
|------|------|------|------|
| **問卷資料** (755 NJ 家戶) | 400 agent 初始化 | 755 → 400 | 經驗 → 合成 |
| **原型** (15 人設) | ICC probing (L3) | 15 | 手動設計 |
| **LLM Probing 回應** | 心理計量運算 | 2,700 | LLM 生成 |

**關鍵區分**：

- **原型不是從問卷資料抽樣的。** 它們是手動設計的，涵蓋人口-情境空間，包括極端案例（如 `resilient_veteran`、`vulnerable_newcomer`）。
- **L3 驗證獨立於實驗。** 它使用原型，而非問卷初始化的 agents。
- **L1/L2 驗證使用實驗軌跡（52,000 個決策）。** 不需要額外的 LLM 呼叫。
...
│      │                                          │                           │
│      ▼                                          ▼                           │
│  400 Agents ──────────────────┐          2,700 LLM 回應                     │
│      │                        │                 │                           │
│      ▼                        │                 ▼                           │
│  主實驗                        │           L3 驗證                          │
│  (52,000 決策)                │           (ICC, eta², 敏感度)               │
│      │                        │                                             │
│      ▼                        │                                             │
│  L1/L2 驗證 ◄─────────────────┘                                             │
│  (CACR, R_H, EPI)                                                           │
...
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  對每個 (原型, 情境) 組合:                                            │  │
│  │  prompt = 原型.persona + 情境.scenario + 原型.memory                 │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                              │                                             │
│                              ▼                                             │
│  步驟 4: LLM 呼叫 (每個 30 次重複) ─────────────────────────────────────── │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  模型: gemma3:4b, 溫度: 0.7                                          │  │
│  │  總呼叫: 15 × 6 × 30 = 2,700                                         │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
...
│  步驟 5: 解析回應 ──────────────────────────────────────────────────────── │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  提取: TP_LABEL, CP_LABEL, decision, reasoning                       │  │
│  │  轉換為序數: VL=1, L=2, M=3, H=4, VH=5                               │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                              │                                             │
│                              ▼                                             │
│  步驟 6: 建構資料矩陣 ──────────────────────────────────────────────────── │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  矩陣形狀: 90 列 (15 原型 × 6 情境) × 30 行                          │  │
│  │                                                                       │  │
...
| 指標 | 目標 | 實際 | 狀態 |
|------|------|------|------|
| ICC(2,1) TP | ≥ 0.60 | **0.964** | ✓ 優秀 |
| ICC(2,1) CP | ≥ 0.60 | **0.947** | ✓ 優秀 |
| eta² TP | ≥ 0.25 | **0.330** | ✓ 大效應 |
| eta² CP | ≥ 0.25 | **0.544** | ✓ 非常大 |
| 人設敏感度 | ≥ 75% | **75%** | ✓ 通過 |
| 提示詞敏感度 | 無偏差 | **OK** | ✓ 通過 |
...
在運行 52,000 次 LLM 呼叫的主實驗之前，我們必須回答：

> **「這個 LLM 可靠嗎？它能產生一致且有意義的回應嗎？」**

如果 LLM 給出隨機答案（低 ICC）或無法區分不同人設（低 eta²），模擬結果就沒有意義。

**我們回答的三個問題**：

| 問題 | 指標 | 「通過」的意義 |
|------|------|----------------|
| 相同人設 → 相同回應？ | ICC ≥ 0.60 | 60%+ 的變異是可靠的 |
| 不同人設 → 不同回應？ | eta² ≥ 0.25 | 人設解釋 25%+ 的變異 |
| 改變人設 → 預期的行為變化？ | 敏感度 ≥ 75% | 75%+ 的交換按預期運作 |
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  問卷資料 (755 受訪者)                  原型 (15 人設)                       │
│  ─────────────────────────────          ─────────────────────────           │
│  • 回答我們 Qualtrics 問卷              • 研究者設計的虛構代表               │
│    的真實人物                             性人物                             │
│                                                                             │
│  • 用途：初始化實驗中的                 • 用途：在實驗前測試                 │
│    400 個 agents                          LLM 可靠性                        │
│                                                                             │
│  • 可能不包含極端案例                   • 必須包含極端案例                   │
│    （非常貧困、殘障）                     (resilient_veteran 等)            │
│                                                                             │
│  • 分布反映實際 NJ 人口                 • 分布最大化原型間變異               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
1. **載入 15 個原型**：每個原型是一個完整的「人設」，包含人口統計、洪災歷史、財務狀況和背景故事（memory_seed）。

2. **載入 6 個情境**：每個情境描述一個洪水場景（低/中/高嚴重性、矛盾訊號、適應後）。

3. **組合它們**：對每個（原型, 情境）配對，建構提示詞：
   - 「你是一個 [原型描述]...」
   - 「上個月，[情境場景]...」
   - 「你的記憶：[memory_seed]...」

4. **呼叫 LLM 30 次**：對 90 個組合（15×6）中的每一個，用 temperature=0.7 呼叫 LLM 30 次。

5. **解析回應**：從每個回應中提取 TP_LABEL、CP_LABEL、decision。

6. **建構資料矩陣**：將 2,700 個回應組織成 90×30 矩陣。
...
- **ICC = 0.964**：當給予相同的（原型, 情境），LLM 有 96% 的時間產生幾乎相同的 TP 標籤。這非常優秀——LLM 不是在隨機猜測。

- **eta² = 0.330**：原型差異解釋了 33% 的 TP 變異。這表示 LLM 成功區分不同的人設（MG vs NMG、曾淹水 vs 從未淹水等）。

- **人設敏感度 = 75%**：當我們將收入從 MG→NMG 交換時，CP 如預期增加。當我們將某人移到安全區域時，TP 減少。LLM 以理論上一致的方式回應人設變化。

**結論**：Gemma 3 4B 足夠可靠，可以運行 52,000 次呼叫的主實驗。
